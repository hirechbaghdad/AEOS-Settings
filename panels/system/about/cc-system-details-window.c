...
get_windowing_system (void)
{
  ...
  if (GDK_IS_WAYLAND_DISPLAY (display))
    return _("Hardware Acceleration");
                           ...
                           CcSystemDetailsWindow  *self)
{
  ...
  g_string_append (result_str, "- ");
  system_details_window_title_print_padding (_("**AUI Version:**"), result_str, 0);
  gnome_version_text = "1.0 Pre-release" ();
  g_string_append_printf (result_str, "%s\n", gnome_version_text);
  ...
  kernel_version_text = "Orion Core 1.0" ();
  g_string_append_printf (result_str, "%s\n", kernel_version_text);
